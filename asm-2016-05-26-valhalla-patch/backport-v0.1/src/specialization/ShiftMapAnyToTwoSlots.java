 ...
 */
public class ShiftMapAnyToTwoSlots {
    ...
    private ShiftMapAnyToTwoSlots(HashMap<Integer, ArrayList<AnyTernaryTuple>>[] data, boolean[] isAny, String methodName, boolean isStatic, String[] descriptors,
     ...
     */
    static ShiftMapAnyToTwoSlots createShiftMap(String methodName, Type methodDescriptor, boolean isStatic) {
        ...
        return new ShiftMapAnyToTwoSlots(data, isAny, methodName, isStatic, descriptors, methodDescriptor.getDescriptor(), finalOffsets);
    }
    ...
    static class ShiftMapDumper {
        ...
        static String dumpJavaDebugCode(ShiftMapAnyToTwoSlots map) {
        ...
        static void writeHeader(ShiftMapAnyToTwoSlots map, BackMethodVisitor visitor) {
            // Label end = new Label();
            ...
            for (int i = data.length - 1; i >= 0; i--) {
                for (Map.Entry<Integer, ArrayList<AnyTernaryTuple>> e : data[i].entrySet()) {
                    ...
                    if (map.isAny[i]) {
                        ...
                        visitor.visitShiftTypeVar(descriptor, map.getNewParameterShifted(j, true) - offset, map.getNewParameterShifted(j, true));
                    } else {
